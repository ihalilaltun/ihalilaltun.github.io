 ...
 * sgmTranformers 
 * @constructor
 * @param {object} obj - Object that VTEX App providing
 * @returns {object} reBuildedObj - Functions will return rebuild objects for Vtex app to send events if sendEvent is true
 */
    window._SgmntfY_.sgmTranformers = {
      'Page': (obj) => {
        //This function is for page view events

        if(obj?.eventType === 'productView') {
          obj.category = 'Product Page';
        }else if(obj.eventType === 'homeView') {
        ...
        //if sendEvent is true, this return need to send Page View event
        return reBuildedObj;
      },
      'Product': (obj) => {
        //This function is for product view events
    ...
    /** 
* reBuildedObj structure is,
* {
* sendEvent: true/false //boolen value that if, it is true, Vtex app will send an event, if it is false then Vtex app won't send any event to Segmentify System.
* newObj: {} //object value that functions will return rebuild objects for Vtex app to send events if sendEvent is true
* }
*/
  } catch (e) {}
}
  ...
  var _SgmntfY_ = window['_SgmntfY_'] = {
      ...
      recommendProducts: function (params, request) {
        ...
        function checkSelectorAndRender() {
          ...
          if (!target || !_SgmntfY_._getJq()(target).length) {
            ...
            if(params['cssCode']) _SgmntfY_._getJq()('<style />').html(params['cssCode']).prependTo(_SgmntfY_._getJq()('body'));
      ...
      if (currentStep) {
        ...
        if (
          ...
          _SgmntfY_
            ...
            .prependTo(
              _SgmntfY_._getJq()(_SgmntfY_._variables.journey.wrapperSelector),
            );
    ...
    _campaigns: {
            ...
            if ('serviceWorker' in navigator) {
              navigator.serviceWorker
                ...
                .catch(function (e) {
                  _SgmntfY_.LOG_MESSAGE(
                    'ERROR',
                    'Error while registering service worker in vapid method' + e,
                  );
                });
            }
          }
      ...
      WHEEL_OF_FORTUNE: function (campaign) {
        ...
        var winModalHeader = document.querySelector('.seg-wof-win-bg .seg-wof-header');
        ...
        var wheelRotateContainer = document.querySelector('.sfy-wheel-container');
        ...
        function drawWheelBase() {
          ...
          contentOfButton.setAttribute('style', 'background-color: ' + config.contentButtonColor + '!important');
        ...
        function frame() {
          ...
          if (!isReadyToFinish) {
            ...
            finishDegree = (config.reward.index * angleForOneSlice +
                differenceDegreeForFinish) * -1;
      ...
      SCRATCH: function (campaign) {
        ...
        function getMobileOffset(event) {
          ...
          var _offsetX = event.touches[0].clientX - window.pageXOffset - _rect.left;
          var _offsetY = event.touches[0].clientY - window.pageYOffset - _rect.top;
    ...
    _functions: {
      ...
      userIysPermissions: function (data) {
        ...
        var emailIysSource = _SgmntfY_._variables.ua.type === 'PC' ? 'HS_WEB' : 'HS_MOBIL';
    ...
    _prepareRequest: function (params, eventName, consumeNextPage) {
      ...
      var request = {
        data: {
          ...
          pageUrl: _SgmntfY_._getRequestParam(params['pageUrl'], document.URL, false),
          referrer: _SgmntfY_._getRequestParam(params['referrer'], document.referrer, false),
          browser: _SgmntfY_._getRequestParam(params['browser'], _SgmntfY_._variables.ua.name, false),
          os: _SgmntfY_._getRequestParam(params['os'], _SgmntfY_._variables.ua.os, false),
          osversion: _SgmntfY_._getRequestParam(params['osversion'], _SgmntfY_._variables.ua.osversion, false),
          userAgent: _SgmntfY_._getRequestParam(params['userAgent'], _SgmntfY_._variables.ua.agentString, false),
          lang: _SgmntfY_._getRequestParam(params['lang'], _SgmntfY_._variables.language, true),
          currency: _SgmntfY_._getRequestParam(params['currency'], _SgmntfY_._variables.currency, true),
          region: _SgmntfY_._getRequestParam(params['region'], _SgmntfY_._variables.region, true),
    ...
    _searchSetActiveRootElement: function () {
      ...
      function onIterateShadowRootElementsForTrigger(shadowRootEl) {
        ...
        if (rootEl && rootEl.length > 0) {
          ...
          _SgmntfY_.LOG_MESSAGE(
            'WARN',
            'ShadowRoot Element For Trigger could not be found: ' + shadowRootEl,
          );
        }
      }
    },
    ...
    _searchResultHandler: function (request, searchResult) {
      ...
      if (products !== null && products.length > 0 || 
        noResultSearch !== null && noResultSearch.recommendedProducts && noResultSearch.recommendedProducts.length > 0) {
        _SgmntfY_._variables.segmentifyObj('event:interaction', {
          type: 'widget-view',
          interactionId: 'static',
          instanceId: campaign['instanceId'],
      ...
      _SgmntfY_._getJq().each(searchAssetElements, function (index, assetItem) {
        if (assetItem['type'] === 'CATEGORY') {
          ...
          sgmSearchSettings.isSidebarItemsClickable.lastSearches = assetItem['clickable'];
          sgmSearchSettings.searchHistoryClearSettings.clearButtonText =  assetItem['clearButtonText'];
          sgmSearchSettings.searchHistoryClearSettings.clearButtonEnabled = assetItem['clearButtonEnabled'];
          sgmSearchSettings.searchHistoryClearSettings.clearQueryBasedEnabled = assetItem['clearQueryBasedEnabled'];
        }
      });
        ...
        .each(
          ...
          function (language, assetText) {
            if (language === request.data.lang) {
              ...
              sgmSearchSettings.texts.lastSearchesText = assetText['lastSearchesText'];
      ...
      if (products.length) {
        ...
        sgmSearchSettings.data['products'] =
          _SgmntfY_._getArrayOfProductMap(noResultSearch.recommendedProducts);
      ...
      sgmSearchSettings.data.searchHistoryClearSettings = sgmSearchSettings.searchHistoryClearSettings;
      ...
      function addNoResultIcon() {
        try {
          ...
          var noResultIconTargetElem = document.querySelector('.sgm-search-products-title-container');
          ...
          _SgmntfY_.LOG_MESSAGE(
            'WARN',
            'Error in adding no search result icon in after search: ', + err,
          );
        }
      }
      ...
      function noResultStyle() {
        var noResultContainer = _SgmntfY_._getJq()(sgmSearchSettings.searchResultsEl);
        noResultContainer.find('ul a').css('width', 'calc(50% - 6px)');
        noResultContainer.find('ul a:nth-child(odd)').css('margin-right', '10px');
        noResultContainer.find('ul a:nth-child(even)').css('margin-right', 'unset');
    ...
    GA: {
      ...
      convertAndSend: function (_fields) {
        ...
        if (isGA4Active) {
          fields = {
            eventAction: 'sgm_' + _fields.action.toLowerCase().replaceAll(' ','_'),
        ...
        if (_fields.sgmProduct && _SgmntfY_._variables.ga.dimensions.sgmProduct) {
      ...
      sendEvent: function (request) {
        ...
        if (!request.eventCategory && !request['event_category'] && !_SgmntfY_._variables.ga.ga4) {
        ...
        if (gaActivated) {
          if (_SgmntfY_._variables.ga.ga4) {
            ...
            const requestWithoutEventAction = {...request};
            delete requestWithoutEventAction.eventAction;
            _SgmntfY_._variables.ga.tracker('event',eventAction , requestWithoutEventAction);
          } else {
            // send prepared event-handlers to google-analytics with related/selected tracker
    ...
    init: function () {
      ...
      if (
        ...
        if (_SgmntfY_._variables.doNotTrack === false && _SgmntfY_._variables.pa.loaded === false) {
          _SgmntfY_._loadJavascript(
